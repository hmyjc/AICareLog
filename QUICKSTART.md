# 健康档案助手 - 快速开始指南

## 项目概述

这是一个完整的健康档案助手系统，包含后端API服务和前端React应用。

## 目录结构

```
health_agent/
├── backend/                # 后端代码
│   ├── api/               # API接口
│   │   ├── health_profile.py  # 健康档案管理
│   │   ├── persona.py         # 人物风格管理
│   │   └── push.py            # 健康推送
│   ├── config/            # 配置文件
│   │   ├── config.yaml    # 主配置文件
│   │   └── config.py      # 配置加载
│   ├── core/              # 核心功能
│   │   └── mongodb.py     # MongoDB连接
│   ├── models/            # 数据模型
│   │   └── health_profile.py
│   ├── services/          # 业务服务
│   │   ├── llm_service.py     # 大模型服务
│   │   └── push_service.py    # 推送服务
│   ├── scheduler/         # 定时任务
│   │   └── tasks.py
│   ├── utils/             # 工具函数
│   │   ├── persona_styles.py  # 人物风格配置
│   │   └── weather_crawler.py # 天气爬虫
│   └── app.py             # 主应用入口
├── frontend/              # 前端代码
│   ├── public/
│   ├── src/
│   │   ├── components/    # 组件
│   │   ├── pages/         # 页面
│   │   │   ├── ProfilePage.tsx      # 用户档案页面
│   │   │   ├── PersonaPage.tsx      # 人物风格页面
│   │   │   ├── RestPage.tsx         # 作息提醒页面
│   │   │   ├── MealPage.tsx         # 饮食提醒页面
│   │   │   ├── WeatherPage.tsx      # 天气推送页面
│   │   │   └── HealthTipPage.tsx    # 养生妙招页面
│   │   ├── services/      # API服务
│   │   ├── store/         # 状态管理
│   │   └── App.tsx
│   └── package.json
├── requirements.txt       # Python依赖
├── README.md             # 项目说明
└── QUICKSTART.md         # 快速开始指南
```

## 安装步骤

### 1. 环境要求

**后端：**
- Python 3.8+
- pip

**前端：**
- Node.js 14+
- npm 或 yarn

### 2. 后端安装

#### Windows系统：

```bash
# 1. 进入项目目录
cd D:\PythonProject\heathy_agent\health_agent

# 2. 安装Python依赖
pip install -r requirements.txt

# 3. 配置数据库
# 编辑 backend/config/config.yaml，确认MongoDB和API配置正确

# 4. 启动后端服务（方式一：使用脚本）
start_backend.bat

# 或方式二：手动启动
cd backend
python app.py
```

#### Linux/Mac系统：

```bash
# 1. 进入项目目录
cd /path/to/health_agent

# 2. 安装Python依赖
pip3 install -r requirements.txt

# 3. 配置数据库
# 编辑 backend/config/config.yaml

# 4. 启动后端服务
chmod +x start_backend.sh
./start_backend.sh

# 或手动启动
cd backend
python3 app.py
```

后端服务将在 `http://39.104.28.40:8000` 启动

### 3. 前端安装

#### Windows系统：

```bash
# 1. 进入前端目录
cd D:\PythonProject\heathy_agent\health_agent\frontend

# 2. 安装pnpm（如果未安装）
npm install -g pnpm

# 3. 安装依赖
pnpm install

# 4. 启动前端（方式一：使用脚本）
cd ..
start_frontend.bat

# 或方式二：手动启动
cd frontend
pnpm dev
```

#### Linux/Mac系统：

```bash
# 1. 进入前端目录
cd frontend

# 2. 安装pnpm（如果未安装）
npm install -g pnpm

# 3. 安装依赖
pnpm install

# 4. 启动前端
chmod +x ../start_frontend.sh
../start_frontend.sh

# 或手动启动
pnpm dev
```

前端应用将在 `http://39.104.28.40:3000` 启动

## 使用流程

### 1. 创建健康档案
1. 访问 `http://39.104.28.40:3000`
2. 点击"用户档案"菜单
3. 填写基础信息、健康信息和其他信息
4. 点击"创建档案"按钮

### 2. 选择人物风格
1. 完成档案填写后，点击"人物风格"菜单
2. 浏览8种不同的推送风格
3. 点击选择您喜欢的风格

### 3. 设置地区（天气推送）
1. 点击"天气推送"菜单
2. 选择您所在的省份和城市
3. 点击"保存地区"

### 4. 获取健康推送

#### 作息提醒
- 自动推送：每天7:00、13:00、23:00
- 手动获取：点击"作息提醒"菜单，选择时间段获取

#### 饮食提醒
- 自动推送：每天7:30、12:00、18:00
- 手动获取：点击"饮食提醒"菜单，选择餐次获取

#### 天气推送
- 自动推送：每天7:00（需先设置地区）
- 手动获取：点击"天气推送"菜单，点击"获取今日天气"

#### 养生妙招
- 自动推送：每天14:00
- 手动获取：点击"养生妙招"菜单，点击"获取今日养生妙招"

## API文档

后端启动后，访问以下地址查看完整API文档：
- Swagger UI: `http://39.104.28.40:8000/docs`
- ReDoc: `http://39.104.28.40:8000/redoc`

## 主要API接口

### 健康档案
- `POST /api/health-profile` - 创建健康档案
- `GET /api/health-profile/{user_id}` - 获取健康档案
- `PUT /api/health-profile/{user_id}` - 更新健康档案
- `POST /api/health-profile/{user_id}/location` - 设置用户地区

### 人物风格
- `GET /api/persona-styles` - 获取所有人物风格
- `POST /api/persona-styles/{user_id}/select` - 选择人物风格
- `GET /api/persona-styles/{user_id}/current` - 获取当前风格

### 健康推送
- `POST /api/push/rest/{user_id}` - 推送作息提醒
- `POST /api/push/meal/{user_id}` - 推送饮食提醒
- `POST /api/push/weather/{user_id}` - 推送天气信息
- `POST /api/push/health-tip/{user_id}` - 推送养生妙招
- `GET /api/push/history/{user_id}` - 获取推送历史

## 配置说明

### 后端配置（backend/config/config.yaml）

```yaml
# 阿里云百炼配置
dashscope:
  url: "https://dashscope.aliyuncs.com/compatible-mode/v1"
  api_key: "your-api-key"  # 替换为您的API密钥
  model: "qwen-max"

# MongoDB配置
mongodb:
  uri: "mongodb://user:password@host:port/admin"  # 替换为您的MongoDB连接
  database: "arbismart"

# 定时任务配置
scheduler:
  timezone: "Asia/Shanghai"
  rest_times: ["07:00", "13:00", "23:00"]
  meal_times: ["07:30", "12:00", "18:00"]
  weather_time: "07:00"
  health_tip_time: "14:00"
```

### 前端配置（frontend/.env）

创建 `.env` 文件（或复制 `.env.example`）：

```
VITE_API_URL=http://39.104.28.40:8000/api
```

## 常见问题

### 1. 后端启动失败
- 检查Python版本是否为3.8+
- 检查所有依赖是否正确安装
- 检查MongoDB连接是否正常
- 检查8000端口是否被占用

### 2. 前端启动失败
- 检查Node.js版本是否为14+
- 删除node_modules文件夹，重新运行`npm install`
- 检查3000端口是否被占用

### 3. API请求失败
- 检查后端服务是否正常运行
- 检查前端.env文件中的API_URL配置
- 查看浏览器控制台和后端日志

### 4. 定时任务不执行
- 确认后端服务正在运行
- 检查日志确认调度器是否启动
- 确认系统时区设置正确

## 技术支持

如有问题，请查看：
1. 项目README.md文档
2. API文档 (http://39.104.28.40:8000/docs)
3. 后端日志文件

## 开发建议

- 使用虚拟环境管理Python依赖
- 定期备份MongoDB数据库
- 生产环境请修改配置中的密钥和连接信息
- 建议使用Git管理代码版本

---

祝您使用愉快！🏥

